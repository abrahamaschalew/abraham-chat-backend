  ...
  user.save((err, result) => {
    ...
    mongo.user.find({ username: req.body.username, password: req.body.password },
      (err, result) => {
        ...
        if (result.length == 0)
          return res.status(404).json({ msg: "Couldn't find user on the given username  & password" });
        ...
        if (result.length == 1) {
          ...
          return res.status(404).json({ msg: "Couldn't find account on the given username & password" });
        }
      })
  })
}
